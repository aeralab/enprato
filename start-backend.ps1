...
$port = if ($env:ENPRATO_BACKEND_PORT) { $env:ENPRATO_BACKEND_PORT } else { '18788' }
...
Write-Host "Backend https://0.0.0.0:$port (phone mic needs https)"
& $py -m uvicorn backend.app.main:app --host 0.0.0.0 --port $port --ssl-certfile $cert --ssl-keyfile $key
